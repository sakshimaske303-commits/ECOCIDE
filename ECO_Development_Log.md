...
Decided I wasnt jst going to patch this quietly nd move on. Wrote `export_zone_geometries.py` to simplify each zone's real polygon down to something Sentinel Hub's Statistics API would actually accept in one request (the Danube Delta shapes especially are insanely detailed, Kherson alone came in at 28,406 raw vertices), nd `download_ndvi_polygon.py` to query the true simplified polygon directly instead of the bbox, plus a proper per pixel SCL mask in the evalscript excluding no data, saturated, cloud shadow, cloud, cirrus, nd snow/ice pixels. Output went to a brand new `data/ndvi_v2/` folder so the old bbox based numbers stayed untouched as a baseline to compare against, nothing got overwritten blind.

Ran it. All 5 zones came back with their full 35 monthly points, so no API failures at least. But the comparison against the old data looked genuinely alarming at first glance: valid pixel percentage per zone had collapsed hard, like Kherson's best month capped out around 51% valid instead of nearly 100%, nd the mean NDVI had shifted up almost everywhere by anywhere from 0.05 to 0.15. My first instinct was tht I'd broken something in the evalscript. So I refused to jst accept either explanation (new bug, or new corruption) without checking, nd went nd built a tiny standalone script tht computes each zone's polygon area against its own bounding box area using jst the raw shoelace formula, since I dont even have shapely available in every environment I was checking this from. The area ratios came back almost exactly matching the observed valid% ceiling for every single zone (Kherson ~0.50, Tulcea ~0.66, Galați ~0.66, Constanța ~0.43, Brăila ~0.65). Tht's nt a coincidence, tht's jst wht happens when you clip to the real polygon instead of a rectangle: the pixels sitting inside the bbox but outside the actual shape get correctly marked no data now, when they weren't being excluded before. So this part is a correct fix doing exactly wht it's supposed to do, not a new bug I introduced.

Then came the part I actually didnt want to find. Reran the whole statistical battery — main DiD, both placebos, four county panel, per control breakdown, event study, placebo in space, control only spillover, lag length nd functional form checks, low coverage sensitivity — against the corrected data. The main result moved from -0.0703 (HAC p=0.022) to -0.0747 (HAC p=0.060). Direction's the same, size is basically the same, but it no longer clears the 0.05 line under its own preferred standard errors. Four county panel did the same thing, -0.0600/p=0.029 to -0.0661/p=0.059.

But the real gut punch was the placebo in space check. Under the old bbox data, Kherson was rank 1 of 5 in the randomization test, the best possible score you can even get with only five units (exact p=0.20). Under the corrected data it drops to rank 2 one sided nd rank 3 two sided (exact p=0.40 to 0.60), because Brăila nd Constanța, jst sitting there as ordinary control counties tht nvr had a dam break near them, show effects under the identical fake treatment procedure tht are as big or bigger than Kherson's real one. Ran the control only spillover check again too (Kherson pulled completely out of the panel) nd now two of the four controls move significantly on their own instead of jst one — Constanța was already flagged before, but now Brăila joins it, both showing their own independent shift around the same June 2023 window for reasons I genuinely dont know nd this design cant tell me.

Before jst accepting tht the result is weaker now, I did the one sanity check tht felt like it could let me off the hook: maybe it's jst a handful of extra low coverage months in the new data dragging the number down, since the whole valid% ceiling dropped structurally anyway. Dropped everything below 15% valid, then again below 25% valid, nd reran the main model both times. Didnt help. If anything the p value got slightly worse each time (0.060 -> 0.070 -> 0.083). So no, it's nt a coverage artifact, the weaker result is jst wht the correctly extracted data actually says.

One small honest silver lining in the middle of all this: the narrowed baseline placebo test, which I'd flagged all the way back in Entry 7 and Entry 10 as a genuine validation failure (fake date inside the narrow window coming back significant under HAC, p=0.001), doesnt fail anymore under the corrected data (p=0.069 now). So not every single thing got worse, jst most of it, nd I'm writing down both sides rather than only the bad one.
