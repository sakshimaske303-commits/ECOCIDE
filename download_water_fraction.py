...
def request_water_fraction(access_token, geometry, start, end):
    headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}

    # Instead of averaging NDWI across the whole box (which dilutes the
    # water signal against a much larger area of dry land), classify each
    # pixel as water (NDWI > 0) or not, and report the fraction of water
    # pixels — a direct measure of water extent, not diluted brightness
    evalscript = """
    //VERSION=3
    function setup() {
      return {
...
def main():
    ...
    for zone_name, bbox in ZONES.items():
        ...
        result = request_water_fraction(access_token, geometry, "2023-04-01", "2023-08-31")

        if result:
            output_path = os.path.join(OUTPUT_DIR, f"{zone_name}_water_fraction.json")
