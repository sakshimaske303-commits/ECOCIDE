...
| Variable | Source |
|---|---|
| NDVI (monthly) | Sentinel-2, Sentinel Hub Statistical API (monthly aggregate, cloud coverage ≤40%) |
...
Section 4.5 asks whether the Kherson effect survives against each control zone individually, with Kherson always held fixed as the treated unit. That is not quite the same question a skeptical reader would ask with only five geographic units on the table: if one of the four Romanian counties, rather than Kherson, had been the one assigned "treated" status at the real June 2023 cutoff, would it have shown an effect just as large? Cluster-robust or HAC p-values computed from five clusters cannot rule this out on their own (Conley & Taber, 2011); an exact randomization test can, so I ran one directly.

Each of the five zones was assigned "treated" status in turn, against the other four as its control panel, using the same real post-June-2023 cutoff — a placebo in space (who is treated) rather than in time (when treatment happens, already covered in Section 4.4). Kherson's own coefficient (did_term = −0.0600, HAC p = 0.030) turns out to be the single most extreme of the five, both by signed value (most negative) and by absolute size. Ranked against the other four possible assignments, Kherson is 1st of 5 either way, which gives an exact randomization p-value of 1/5 = 0.20 — the smallest randomization p-value any single unit could ever obtain in a five-unit panel, since there are only five possible assignments to rank against. That ceiling, not the strength of the asymptotic HAC p-value reported elsewhere in this paper, is the honest limit on how confidently this result can be stated from a design with this few clusters.

This check also surfaces something the per-zone breakdown in Section 4.5 does not, on its own: two of the four placebo (fake-treated) assignments return coefficients close in size to Kherson's real one. Brăila, assigned fake-treated status, returns did_term = +0.0571 (HAC p = 0.018) — a significant effect, though opposite in sign to Kherson's decline — and Constanța returns −0.0520 (HAC p = 0.099), same sign as Kherson's and only just short of conventional significance. Tulcea and Galați, the remaining two placebo assignments, both come back small and non-significant (+0.028 and +0.027). I report this directly rather than smoothing it over: with only five clusters it is not rare for at least one of them to throw off a large or "significant" coefficient from panel noise alone, which is exactly the caveat about few-cluster inference already raised in Sections 4.5 and 6 — this check simply gives that caveat an actual number instead of leaving it qualitative.
...
Applying a Bonferroni correction (family-wise α=0.05 across 11 tests, corrected threshold p<0.00455) leaves only two of the four quarters flagged in Section 4.3 standing: the already-disclosed pre-treatment quarter (summer 2022, p=0.0009) and the one-year-post-treatment quarter (p<0.0001). The treatment-quarter coefficient itself (p=0.0076) and the following-quarter coefficient (p=0.028) do not survive this correction. Applying the less conservative Benjamini–Hochberg false-discovery-rate procedure (Benjamini & Hochberg, 1995) at q=0.05 recovers the treatment-quarter result (rank 3 of 11, corrected threshold 0.0136) but not the following-quarter result (rank 4 of 11, threshold 0.0182), which stays unsupported under either correction.

The practical read: the immediate treatment-quarter effect holds up under a standard false-discovery-rate correction but not under the stricter Bonferroni bound, and the following-quarter effect — already the less central of the two claims in Section 4.3 — does not clear either bar. This does not change the paper's central specification (Section 4.2, the pooled pre/post DiD, which is a single test and is not subject to this correction); it does mean the finer-grained quarter-by-quarter story in Section 4.3 should be read with this caveat attached, rather than treating all four originally-flagged quarters as equally supported.

### 4.8 Spillover Check Among Control Zones

Section 6 argues that spatial spillover into the control counties is unlikely, but only on distance grounds (~350 km, across an international border) — a plausibility argument, not a test. I ran the direct test the existing data allows: with Kherson excluded from the panel entirely, each of the four Romanian counties was in turn assigned "treated" status against the other three, at the same real June 2023 cutoff. If the war's regional effects — disrupted Black Sea shipping, agricultural-market disruption, and so on — had spilled into the ostensibly unaffected controls, at least one of these four control-only comparisons should show a shift even with Kherson removed.

Tulcea (did_term = +0.0137, HAC p = 0.642) and Galați (+0.0127, p = 0.691) come back clean, with no detectable shift relative to the other controls. Brăila is borderline (+0.0449, p = 0.069). Constanța, however, shows a significant negative shift relative to the other three (−0.0714, HAC p = 0.033) — a decline close in sign and magnitude to Kherson's own real effect, occurring entirely among counties that were never flooded.

This gives the Constanța anomaly already disclosed in Section 4.5 an actual explanation rather than leaving it as an unresolved "ecological difference": Constanța appears to have undergone its own independent post-June-2023 decline, for a reason unrelated to the Kakhovka Dam, and that decline is the most likely reason the pairwise Kherson-vs-Constanța comparison in Section 4.5 came back close to zero — both zones were declining, for unconnected reasons, over the same window, which understates rather than eliminates the true Kherson effect in that one specific pairing. Because the primary specification (Section 4.2) uses Tulcea, and Tulcea passes this control-only spillover check cleanly, this does not weaken the paper's headline result; it does mean Constanța specifically, not the control panel generally, should be read with caution, and its presence in the four-county pooled panel (Section 4.5) is now a documented, tested source of attenuation rather than a purely speculative one.

### 4.9 Specification Robustness: Lag Length and Functional Form

Two further analyst choices in the primary specification (Section 3.3) were made once and reported without themselves being stress-tested: the Newey–West lag length, and the linear functional form of the outcome variable. Both were re-examined directly against the primary Kherson-vs-Tulcea model.

**Lag length.** The main specification uses maxlags=3. Re-estimating the identical model at maxlags=1 through 6 leaves the did_term coefficient unchanged, as it must (HAC affects only the standard errors, not the point estimate), and its p-value stable throughout the whole range (0.011–0.026), never approaching non-significance. The lag-length choice is not doing any of the work in this result.

**Functional form.** The primary specification models raw NDVI linearly. Re-estimating the identical specification on log(NDVI) instead returns a similarly-signed effect (a roughly 49% proportional decline) but a weaker p-value (0.055 versus 0.022–0.026 for the linear specification) — just missing the conventional 0.05 threshold. I report this directly rather than only reporting the specification that clears significance: NDVI's near-zero values in the most cloud-contaminated months (Section 6) become proportionally much larger swings once log-transformed, so the log specification is more sensitive to precisely the noisiest observations in the panel. This does not overturn the primary result, which is linear and matches the scale on which NDVI is conventionally interpreted (Section 4.2), but the finding is not perfectly robust to this specific functional-form choice, and that qualification is recorded here rather than left for a reviewer to find.
...
- **Spatial spillover.** The Difference-in-Differences design makes the assumption that there is no impact of the treatment on the control zone. The ~350 km distance between Kherson and the Romanian counties, across an international border, was originally the only argument offered for this assumption. Section 4.8 now tests it directly, excluding Kherson and checking whether any control county shifts relative to the others after June 2023: Tulcea and Galați pass cleanly, Brăila is borderline, and Constanța does not — it shows its own independent post-event decline, unrelated to the dam. The assumption holds for the county used in the primary specification (Tulcea) and for the multi-control panel generally, but not uniformly across all four, and Constanța's role in Section 4.5's pooled estimate should be read with that in mind.
- **Functional-form sensitivity.** The primary specification models NDVI linearly. Section 4.9 shows the same effect under a log(NDVI) transformation is directionally the same but weaker (p=0.055, versus 0.022–0.026 for the linear specification) — likely driven by cloud-contaminated near-zero NDVI months being disproportionately amplified in log space. The result is not fully independent of this modeling choice.
- **Data loss due to cloud contamination.** It is well-documented in the ECO_Development_Log.md, that for optical Sentinel-2 composites cloud cover can introduce bias as to which pixel may lead to an effective contribution to a given month's statistic. The use of multi-sensor (including radar) flood product for the flood-extent analysis was a motivation for this, whereas the NDVI series itself, as an optical product, shares this general limitation and is partially alleviated by aggregating the data at a monthly time scale (as opposed to a higher frequency). Concretely, in the worst-affected month in the Kherson series (December 2022), up to 68.6% of pixels in the monthly aggregate returned no data because of cloud cover — the scale of this limitation stated directly rather than only in the abstract.
- **Monthly aggregation vs. exact event date.** The dam was destroyed on 6 June 2023, but NDVI is aggregated at monthly resolution, so the "June 2023" observation pools 5 pre-event days (1–5 June) together with 25 post-event days into a single monthly value. This coarsening dilutes rather than inflates the post-treatment signal — if it biases the estimate at all, it would bias it toward zero, not away from it — and is disclosed here explicitly rather than left implicit in the choice of a monthly design.
...
- **Few-cluster inference.** The few-cluster-inference leverage is only for five clusters where we are below 30-40+, and even at that cluster number, the cluster-robust standard errors are numerically degenerate as is common in the quarterly event study analysis in that model, which is why we report HAC cluster-robust standard errors for that model. Section 4.6 quantifies this concern directly with a placebo-in-space / randomization-inference check: across the five possible unit-assignments, Kherson's own effect is the most extreme, but the best possible exact randomization p-value obtainable with only five units is 0.20 — a ceiling on inferential confidence that no amount of additional modeling can overcome without more clusters (Section 7).
- **Multiple testing in the event study.** The quarterly event study (Section 4.3) tests eleven coefficients against one reference quarter. Section 4.7 reports what survives Bonferroni and Benjamini–Hochberg correction for that multiplicity, rather than treating each raw per-quarter p-value in Section 4.3 as if it were an independent, uncorrected test.
