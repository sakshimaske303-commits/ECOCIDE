...
def main():
    ...
    for year in range(2017, 2025):
        ...
        if os.path.exists(path):
            done = xr.open_dataset(path).attrs.get("items", "")
            if all(i.endswith(str(year)) for i in done.split(",") if i):
                print(year, "exists")
                continue
            print(year, ": existing file mixed in another year's map; recomputing")
            os.remove(path)
        # an item's time range is [1 Jan, 1 Jan next year], so a year search also returns the
        # previous year's map; keep only items whose id ends with the year
        items = [i for i in client.search(collections=[COL], bbox=WINDOW, datetime=f"{year}-01-01/{year}-12-31").items()
                 if i.id.endswith(str(year))]
        if not items:
            print(year, ": no map for this year in the catalogue (not published) — the nearest earlier year will be used")
            continue
        parts = []
