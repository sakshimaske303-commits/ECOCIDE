...
files for Claude to inspect (they are safe to share; no credentials):
    ...
    python v2/05b_check_and_sample.py   (fixed copy of 05_check_and_sample.py)
"""
import glob
import json
import os
...
def main():
    ...
    if os.path.exists(wc_path):
        ...
        if files:  # same grid; copy MODIS coordinates to remove ~1e-9 m float round-off
            ref = xr.open_dataset(files[0])
            wc = wc.assign_coords(x=ref.x.values, y=ref.y.values)
    ...
    if samples:
        samples = [x.drop_vars("spatial_ref", errors="ignore") for x in samples]
        s = xr.concat(samples, dim="time", data_vars="all", join="exact")
        if wcs is not None:
            s = xr.merge([s, wcs.drop_vars("spatial_ref", errors="ignore")], join="exact", compat="override")
        s["spatial_ref"] = xr.DataArray(0, attrs={"crs_wkt": grid.CRS, "note": "EPSG:3035 (see v2/grid.py)"})
        s.to_netcdf(f"{D}/sample_kherson.nc", encoding={v: {"zlib": True, "complevel": 4} for v in s.data_vars if v != "spatial_ref"})
